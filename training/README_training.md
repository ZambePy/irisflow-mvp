...
```
Siga o fluxo normal de calibração do IrisGazeNet (6 pontos). O modelo é salvo automaticamente ao final.
...
```json
{
  ...
  "system": {
    "engine": "IrisGazeNet (MobileNetV2 + SVR)",
    "filter": "Deadzone 12px"
  },
  "points": [...],
  "metrics": {
    "mae_pixels": 48.3,
...
```
training/
├── evaluate_accuracy.py    ← benchmark do sistema atual
├── pretrain.py             ← pré-treino do IrisGazeNet ✅
├── evaluate.py             ← avaliação formal SVR vs baseline ✅
├── dataset.py              ← DataLoader MPIIGaze (Annotation Subset) ✅
├── augmentation.py         ← pipeline Albumentations para ELA ✅
├── model.py                ← IrisGazeNet (IrisFeatureExtractor + IrisGazeEstimator) ✅
