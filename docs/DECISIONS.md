...
## ADR-002 — EyeTrax via Adapter, nunca direto na UI *(histórico — EyeTrax removido)*

**Decisão original:** EyeTrax isolado em `integrations/eyetrax/adapter.py`.  
**Motivo:** Evitar acoplamento — permitir trocar o motor de rastreamento sem tocar na UI.  
**Status atual:** EyeTrax removido do `EngineFactory`. O `EyeTraxAdapter` permanece no código como referência histórica mas é inalcançável via factory (retorna `MockGazeEngine` com aviso de log). A engine de produção é o `IrisGazeNetAdapter`. O padrão de isolamento por adapter foi preservado — ver `integrations/irisgazenet/`.
...
## ADR-008 — Calibração delegada ao EyeTrax na Fase 2 *(histórico — supersedido pela Fase 4)*

**Decisão original:** A tela de calibração do MVP chamava a rotina de calibração nativa do EyeTrax 0.4.  
**Motivo:** Implementar calibração própria é complexo. Delegar ao EyeTrax desbloqueava o hardware real mais rápido.  
**Status atual (Fase 4+):** Calibração própria implementada no `IrisGazeNetAdapter.calibrate()` — coleta ~200 amostras em ~6 pontos e retreina SVR-X e SVR-Y on-device em menos de 5 segundos. EyeTrax não é mais usado para calibração.
...
**Status:** O `IrisGazeNetAdapter` é a engine de produção. O `EyeTraxAdapter` foi removido do `EngineFactory`. A troca de engine é feita via `tracking_engine` no perfil do paciente, sem alterar código de UI.  
